...
const OperatorReportA = ({ operator_name, fromDate, toDate }) => {
  ...
  const [tableFilter, setTableFilter] = useState({
    ...
    // Encode operator name and parameters properly
    const encodedOperator = encodeURIComponent(operator_name);
    const encodedFromDate = fromDate ? encodeURIComponent(fromDate) : '';
    ...
    const htmlContent = `<!DOCTYPE html>
      ...
      <body>
        <div class="summary">
          ...
          <p>Operator ID: ${reportData.tableData[0]?.['Operator ID'] || 'N/A'}</p>          <p>Total Production Hours: ${reportData.totalProductionHours?.toFixed(2) || '0.00'}</p>
          <p>Total Non-Production Hours: ${reportData.totalNonProductionHours?.toFixed(2) || '0.00'}</p>
          <p>Total Idle Hours: ${reportData.totalIdleHours?.toFixed(2) || '0.00'}</p>
          <p>Needle Runtime Percentage: ${reportData.needleRuntimePercentage?.toFixed(2) || '0.00'}%</p>
        </div>
        <table>
          <thead>
            <tr>
            ...
            ${reportData.tableData.map(row => 
              `<tr>
                ${[
                  ...
                  'Sewing Speed', 'Stitch Count', 'Needle Runtime'
                ].map(header => {                  const value = row[header] !== undefined ? 
                    (header.includes('%') ? (row[header]?.toFixed(2) || '0.00') + '%' : 
                     typeof row[header] === 'number' ? row[header]?.toFixed(2) || '0.00' : 
                     row[header]) : '0.00';
                  return `<td>${value}</td>`;
                }).join('')}
              </tr>`
  ...
  const totalTodayHours = 
    (reportData.todaySewingHours || 0) + 
    (reportData.todayIdleHours || 0) + 
    (reportData.todayMeetingHours || 0) + 
    (reportData.todayNoFeedingHours || 0) + 
    (reportData.todayMaintenanceHours || 0);
  // Prepare data for Recharts
  const chartData = [
    { name: "Sewing Hours", value: reportData.todaySewingHours || 0, color: "#3E3561" },
    { name: "No Feeding Hours", value: reportData.todayNoFeedingHours || 0, color: "#118374" },
    { name: "Maintenance Hours", value: reportData.todayMaintenanceHours || 0, color: "#F8A723" },
    { name: "Meeting Hours", value: reportData.todayMeetingHours || 0, color: "#E74C3C" },
    { name: "Idle Hours", value: reportData.todayIdleHours || 0, color: "#8E44AD" }
  ...
  return (
    <div className="operator-container">
          ...
          <table>
            ...
            <tbody>
              {currentRows.map((row, index) => (                <tr key={index}>
                  ...
                  <td>{row["Productive Time in %"]?.toFixed(2) || '0.00'}%</td>
                  <td>{row["NPT in %"]?.toFixed(2) || '0.00'}%</td>
                  <td>{row["Sewing Speed"]?.toFixed(2) || '0.00'}</td>
                  <td>{row["Stitch Count"]?.toFixed(2) || '0.00'}</td> 
      ...
      <div className="top-indicators">        <div className="indicator">
          <h4><FaTshirt /> Total Sewing Hours</h4>
          <p>{reportData.totalProductionHours?.toFixed(2) || '0.00'} Hrs</p>
        </div>
        <div className="indicator">
          <h4><FaTools /> Total Non-Production Hours</h4>
          <p>{reportData.totalNonProductionHours?.toFixed(2) || '0.00'} Hrs</p>
        </div>
        <div className="indicator">
       ...
       <div className="summary-tiles">
        <div className="tile production-percentage">
          <p>{reportData.productionPercentage?.toFixed(2) || '0.00'}%</p>
          <span>Production Percentage</span>
        </div>
        <div className="tile average-speed">
          <p>{reportData.averageSewingSpeed?.toFixed(2) || '0.00'}</p>
          <span>Average Sewing Speed</span>
        </div>
        <div className="tile needle-runtime-percentage">
          <p>{reportData.needleRuntimePercentage?.toFixed(2) || '0.00'}%</p>
      ...
      <div className="chart-breakdown-container">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                >
                  ...
                  formatter={(value) => (value || 0).toFixed(2) + " Hrs"} 
        ...
        </div>        <div className="hour-breakdown">          <div className="hour-box">
            <span className="dot production"></span>
            <p>{(reportData.todaySewingHours || 0).toFixed(2)} Hrs: Sewing Hours</p>
          </div>
          <div className="hour-box">
            <span className="dot non-production"></span>
            <p>{(reportData.todayNoFeedingHours || 0).toFixed(2)} Hrs: Needle break Hours</p>
          </div>
          <div className="hour-box">
            <span className="dot idle"></span>
            <p>{(reportData.todayMaintenanceHours || 0).toFixed(2)} Hrs: Maintenance Hours</p>
          </div>
          <div className="hour-box">
            <span className="dot meeting"></span>
            <p>{(reportData.todayMeetingHours || 0).toFixed(2)} Hrs: Rework Hours</p>
          </div>
          <div className="hour-box">
            <span className="dot no-feeding"></span>
            <p>{(reportData.todayIdleHours || 0).toFixed(2)} Hrs: Idle Hours</p>
          </div>
        </div>
      </div>
    </div>
...
export default OperatorReportA;